...
public class SchemaPatch {
    ...
    private static void patch(DataSource dataSource) {
        List<String> statements = List.of(
                ...
                "UPDATE organizations SET modules_csv = 'STUDENTS,CRM,LMS,FEES,PLACEMENT,COMMS,ANALYTICS,WEBSITE,TESTS,STAFF,GROW,ESS' WHERE product_pack = 'FULL_OPS' AND (modules_csv IS NULL OR TRIM(modules_csv) = '')",
                "UPDATE organizations SET product_pack = 'FULL_OPS', modules_csv = 'STUDENTS,CRM,LMS,FEES,PLACEMENT,COMMS,ANALYTICS,WEBSITE,TESTS,STAFF,GROW,ESS' WHERE (product_pack IS NULL OR product_pack = '' OR product_pack = 'FULL_OPS') AND (modules_csv IS NULL OR modules_csv NOT LIKE '%WEBSITE%' OR modules_csv NOT LIKE '%GROW%')",
