...
export function PlatformDashboardPage() {
  const dash = useApi<Dash>("/api/platform/dashboard");
  const d = dash.data;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">Click a card to open the matching institutes.</p>
      </div>
      ...
      <Card title="Next">
        <p className="text-sm text-slate-600">
          Open <Link className="font-medium text-brand" to="/platform/institutes">Institutes</Link> to set a custom price,
          mark payment received, and activate the workspace. Add Niyamstack staff under{" "}
